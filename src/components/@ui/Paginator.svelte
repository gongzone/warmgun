...
<script lang="ts">
	...
	export let select: string = 'select min-w-[150px]';
	/** Set the text for the amount selection input. */
	export let amountText = 'Items';

	// Props (control)
	/** Set the base classes for the control element. */
	export let regionControl: string = 'btn-group';
	/** Provide variant style for the control button group. */
	export let controlVariant: string = 'variant-filled';
	/** Provide separator style for the control button group.  */
	export let controlSeparator: string = '';

	// Props (buttons)
	/** Provide arbitrary classes to the active page buttons. */
	export let active: string = 'variant-filled-primary';
	/*** Set the base button classes. */
	export let buttonClasses: string = '!px-3 !py-1.5 fill-current';
	/**
	 * Set the label for the Previous button.
	 * @type {string}
	 */
	export let buttonTextPrevious: string = leftArrow;
	/**
	 * Set the label for the Next button.
	 * @type {string}
	 */
	export let buttonTextNext: string = rightArrow;
	/**
	 * Set the label for the First button.
	 * @type {string}
	 */
	export let buttonTextFirst: string = leftAngles;
	/**
	 * Set the label for the Last button.
	 * @type {string}
	 */
	export let buttonTextLast: string = rightAngles;
		...
		{#if showNumerals === false}
			...
			<button type="button" class="{buttonClasses} pointer-events-none !text-sm">
				{settings.offset * settings.limit + 1}-{Math.min(
					...
					settings.size
				)}&nbsp;<span class="opacity-50">of {settings.size}</span>
			</button>
		{:else}
			<!-- Numeric Row -->
